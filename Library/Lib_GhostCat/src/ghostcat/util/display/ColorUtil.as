package ghostcat.util.display
{
	...
	public final class ColorUtil
	{
		...
		/**
		 * 16进制颜色值转化成css样式，如果传入的是ARGB,则把alpha信息忽略掉
		 */
		public static function hex2css(color:uint):String
		{
			var c:String = (0x00FFFFFF & color).toString(16).toUpperCase();
		...
		/**
		 * css样式颜色值转化成16进制颜色值，不包括alpha通道信息，css样式支持长格式 #FFFFFF 及短格式 #0F0;
		 */
		public static function css2hex(color:String):uint
		{
			var ucolor:Number;
			...
			if (color.charAt(0) == "#")
			{
				...
				if (color.length == 3)
				{
					var r:String = color.substr(0, 1) + color.substr(0, 1);
					var g:String = color.substr(1, 1) + color.substr(1, 1);
					var b:String = color.substr(2, 1) + color.substr(2, 1);
					color = r + g + b;
				}
				else if (color.length == 6)
				{
					ucolor = parseInt("0x" + color, 16);
					return isNaN(ucolor) ? 0 : uint(ucolor);
				}
				else
				{
					return 0;
