package ghostcat.util.text
{
	...
	public final class TextFieldUtil
	{
		...
		/*public static function autoFontSize(textField:TextField,adjustY:Boolean = false,resetY:Boolean = false):void
		{
			...
			while (firstLine.width + 5 > textField.width - 2) 
			{
		 ...
		 * <p>
		 * modified by Helcarin, last updated at 2014.04.24
		 * <br>
		 * ※注意，如果文本框开启了换行模式(multiline)，为了避免规则的歧义，此方法会强制关闭换行模式※
		 * <br>
		 * 现在为按指定行数来适应，全部交给TextFiled自行处理，当行数或
		 * </p>
		 */
		public static function autoFontSize(textField:TextField,adjustY:Boolean = false,resetY:Boolean = false, numLines:int = 1):void
		{
			var text:String = textField.text;
			var f:TextFormat = textField.getTextFormat();
			var old_size:int = int(f.size);
			...
			if (numLines)
			{
				while (textField.numLines > numLines)
				{
					f = textField.getTextFormat();
					f.size = int(f.size) - 1;
					if (f.size == 0) break;
			...
			var oldH:Number = textField.height;
			
			//textField.height = textField.textHeight + 200;
			textField.autoSize = "left";
			textField.text = text;
			textField.setTextFormat(f);
			
			while(textField.height > oldH)
			{
			...
			textField.autoSize = "none";
			textField.height = oldH / textField.scaleY;
