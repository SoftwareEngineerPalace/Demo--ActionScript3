package ghostcat.display
{
	...
	public class GSprite extends Sprite implements IDisplayObjectContainer
	{
		 ...
		 *
		 */
		public function GSprite(skin:*=null, replace:Boolean=true, replaceSkin:*=null)
		{
			super();
...
//				RootManager.root = root as Sprite;
			...
			setContent(skin, replace, replaceSkin);
		}
...
//		}
		 ...
		 *
		 */
		public function setContent(skin:*, replace:Boolean=true, replaceSkin:*=null):void
		{
			if (skin is String)
			{
				try
			...
			if (replace && skin)
			{
				...
				if (replaceSkin)
				{
					var replaceSkinCls:Class;
					if(replaceSkin is Class)
					{
						replaceSkinCls = replaceSkin;
					}
					else if(replaceSkin is String)
					{
						replaceSkinCls = getDefinitionByName(replaceSkin) as Class;
					}
					if(replaceSkinCls != null)
					{
						var newSkin:DisplayObject=new replaceSkinCls();
						newSkin.visible=skin.visible;
						newSkin.name=skin.name;
						newSkin.transform.matrix=skin.transform.matrix;
						if (oldParent)
						{
							oldParent.removeChild(skin)
						}
						skin=newSkin;
					}
				}
