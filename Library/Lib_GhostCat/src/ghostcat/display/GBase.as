package ghostcat.display
{
	...
	{
		private var _listeners:Vector.<Mapper>;
		
		/**
		 * ToolTip工厂
		 */
		public static var toolTipFactory:IToolTipFactory;
		 ...
		 *
		 */
		public function GBase(skin:*=null, replace:Boolean=true, replaceSkin:*=null)
		{
			this.self=this;
		...
		{
			_toolTip=v;
			// 这里是我后加的，为的是少写一步toolTipObj操作
			if (toolTipFactory != null)
			{
				if(v != null)
				{
					var toolTipObj:*=toolTipFactory.getToolTipObj(v);
					if (toolTipObj != null)
...
//			regEventHandler();
		 ...
		 */		
		public function mapListener(target:IEventDispatcher, type:String, listener:Function, useCapture:Boolean=false, priority:int=0, useWeakReference:Boolean=false):void
		{
			if(target == null || listener == null) return;
			if(_listeners == null)
			{
				_listeners = new Vector.<Mapper>();
			}
			// 判断是否已经有监听了
			for(var i:int = 0, len:int = _listeners.length; i < len; i++)
			{
				var mapper:Mapper = _listeners[i];
				if(mapper.target == target && mapper.type == type && mapper.listener == listener && mapper.useCapture == useCapture && mapper.priority == priority && mapper.useWeakReference == useWeakReference)
				{
					return;
				}
			}
			...
			_listeners.push(new Mapper(target, type, listener, useCapture, priority, useWeakReference));
		}
		 ...
		 */		
		public function unmapListener(target:IEventDispatcher, type:String, listener:Function, useCapture:Boolean=false):void
		{
			// 判断是否已经有监听了
			for(var i:int = 0, len:int = _listeners.length; i < len; i++)
			{
				var mapper:Mapper = _listeners[i];
				if(mapper.target == target && mapper.type == type && mapper.listener == listener && mapper.useCapture == useCapture)
				{
		...
		public function unmapListeners():void
		{
			if(_listeners != null)
			{
				for(var i:int = 0, len:int = _listeners.length; i < len; i++)
				{
					var mapper:Mapper = _listeners.shift();
					mapper.target.removeEventListener(mapper.type, mapper.listener, mapper.useCapture);
					mapper.destory();
				}
				_listeners = null;
			}
...
class Mapper
{
	...
	public function Mapper(target:IEventDispatcher, type:String, listener:Function, useCapture:Boolean, priority:int, useWeakReference:Boolean)
	{
		this.target = target;
		this.type = type;
