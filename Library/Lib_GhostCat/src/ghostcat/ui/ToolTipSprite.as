package ghostcat.ui
{
	...
	public class ToolTipSprite extends GBase
	{
		...
		{
			if (content && tipTarget)
				(content as IToolTipSkin).positionTo(tipTarget);
		}

		private function mouseOutHandler(event:MouseEvent):void
		{
			var newTarget:DisplayObject = findToolTipTarget(event.target as DisplayObject);
			if(newTarget == tipTarget)
			{
				hide();
				tipTarget = null;
			}
		...
		private function findToolTipTarget(displayObj:DisplayObject):DisplayObject
		{
			...
			while (currentTarget && currentTarget.parent != currentTarget)
			{
				...
				{
					if ( (currentTarget as IToolTipManagerClient).toolTip)
					{
						if (onlyWithClasses == null || Util.isIn(cursor,onlyWithClasses))
						{
		...
		private function show(event:TimerEvent):void
		{
			...
			if (tipTarget is IToolTipManagerClient)
			{
				var client:IToolTipManagerClient = tipTarget as IToolTipManagerClient;
				showToolTip(tipTarget,client.toolTip,client.toolTipObj);
			}
		}
