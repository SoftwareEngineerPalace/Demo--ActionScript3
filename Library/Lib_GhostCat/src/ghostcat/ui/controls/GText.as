...
package ghostcat.ui.controls
{
	...
	public class GText extends GBase
	{
		...
		private var _oriSize:int;// 用来记录初始文字字号，防止经过多次autoFontSize后字号越来越小——Raykid
...
//			}
		...
		public function GText(skin:*=null, replace:Boolean=true, separateTextField:Boolean=false, textPadding:Padding=null, replaceSkin:*=null)
		{
			if (!skin)
				skin=defaultSkin;
			...
			_oriSize = int(textField.defaultTextFormat.size);
		}
...
//					_textPadding.adjectRect(textField,this.content);
		...
		{
			data=v ? ("<html>" + /*LanguageManager.getString(v)*/ v + "</html>") : null
//			data = v;
		}
		 ...
		 * @param numLines	自动调节到的行数，默认单行, <=0表示按照文本框的高度适应
		 */
		public function autoFontSize(adjustY:Boolean=false, resetY:Boolean=false, numLines:int = 1):void
		{
			TextFieldUtil.autoFontSize(textField, adjustY, resetY, numLines);
		}
		...
		protected function doWithText(str:String):void
		{
			...
			if (textChangeHandler != null && enabledLangage)
				str=textChangeHandler(str, textField);
