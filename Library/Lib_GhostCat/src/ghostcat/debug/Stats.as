...
package ghostcat.debug
{
	...
	import ghostcat.util.FormatUtil;
	...
	public class Stats extends Sprite
	{
		...
		protected var updateInterval:int = 1000; //刷新时间间隔，ms数
		
		protected var theme:Object = { bg: 0x50000000, ms: 0xffff00, fps: 0x00ff00, mem: 0x00ffff, memmax: 0xff0070 };
		
		/**
		 * <b>Stats</b> FPS, MS and MEM, all in one.
		 *
		 * @param _theme         Example: { bg: 0x202020, ms: 0xC0C0C0, fps: 0x505050, mem: 0x707070, memmax: 0xA0A0A0 }
		 * @param updateInterval	default 1000ms
		 */
		public function Stats(_theme:Object = null, updateInterval:int = 1000):void
		{
			if (_theme)
			{
				if (_theme.bg != null)
			...
			style.setStyle("fps", { color: ColorUtil.hex2css(theme.fps) });
			style.setStyle("ms", { color: ColorUtil.hex2css(theme.ms) });
			style.setStyle("mem", { color: ColorUtil.hex2css(theme.mem) });
			style.setStyle("memMax", { color: ColorUtil.hex2css(theme.memmax) });
		...
		{
			graphics.beginFill(0x00FFFFFF & theme.bg, ((0xFF000000 & theme.bg) >>> 24) / 256); //分别取出颜色值跟Alpha值
			graphics.drawRect(0, 0, WIDTH, HEIGHT);
			graphics.endFill();
			
			addChild(text);
			
			graph.bitmapData = new BitmapData(WIDTH, HEIGHT - 50, true, 0x00FFFFFF);
			addChild(graph);
		...
		private function update(e:Event):void
		{
			...
			if (timer - updateInterval > ms_prev)
			{
				var realInterval:Number = timer - ms_prev;
				ms_prev = timer;
				...
				graph.bitmapData.fillRect(rectangle, 0x00000000);
				graph.bitmapData.setPixel32(graph.width - 1, graph.height - fps_graph, 0xFF000000 | theme.fps);
				graph.bitmapData.setPixel32(graph.width - 1, graph.height - ((timer - ms) >> 1), 0xFF000000 | theme.ms);
				graph.bitmapData.setPixel32(graph.width - 1, graph.height - mem_graph, 0xFF000000 | theme.mem);
				graph.bitmapData.setPixel32(graph.width - 1, graph.height - mem_max_graph, 0xFF000000 | theme.memmax);
				
				xml.fps = "FPS: " + Math.min(int(fps / realInterval * 1000), stage.frameRate) + " / " + stage.frameRate;
				xml.mem = "MEM: " + FormatUtil.formatBytes(mem, 100); //两位小数
				xml.memMax = "MAX: " + FormatUtil.formatBytes(mem_max, 100); //两位小数
